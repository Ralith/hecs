    ...
    Ok(quote! {
        const _: () = {
            ...
            impl<'a> ::hecs::Query for #ident<'a> {
                ...
                unsafe fn get<'q>(meta: &[::hecs::EntityMeta], fetch: &Self::Fetch, n: usize) -> Self::Item<'q> {
                    #(
                        let #intermediates: <#queries as ::hecs::Query>::Item<'q> = <#queries as ::hecs::Query>::get(meta, &fetch.#fields, n);
                    )*
    ...
    for variant in &data.variants {
        ...
        query_get_variants.extend(quote! {
            Self::Fetch::#ident { #(#named_fields),* } => {
                #(
                    let #named_fields: <#queries as ::hecs::Query>::Item<'q> = <#queries as ::hecs::Query>::get(meta, #named_fields, n);
                )*
