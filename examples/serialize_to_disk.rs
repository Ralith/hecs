...
mod serialize_to_disk_example {
   ...
   pub use hecs::serialize::column::{deserialize_column, try_serialize, try_serialize_id, DeserializeContext, SerializeContext};
   ...
   impl SerializeContext for SaveContext {
      ...
      fn serialize_component_ids<S: serde::ser::SerializeTuple>(
         ...
         try_serialize_id::<ComponentA, _, _>(archetype, &ComponentId::TestComponent1, &mut out)?;
         try_serialize_id::<ComponentB, _, _>(archetype, &ComponentId::TestComponent2, &mut out)?;
         out.end()
      }
...
pub fn main() {
   ...
   let input_data2 = ComponentB { some_other_data: "Hello".to_string() };
