...
//!
//! Both formats reinstate entities at their serialized handles (row through
//! `spawn_at`, column through `spawn_column_batch_at`), so a round trip must
//! preserve the exact `Entity` handles (id and generation) as well as the
//! components. Malformed input is the other half: these are the only entry
//! points that build a `World` from untrusted input, so the properties here
//! assert that a stream that is not a valid serialization is rejected rather
//! than accepted or panicked on, and that a rejected stream does not leak the
//! components it had already parsed. `D` is drop-tracked and deserialized
//! through its own constructor, which is what makes the leak half checkable.
...
impl row::SerializeContext for Row {
    ...
    /// Length-prefixed formats such as bincode need this. `len` counts every
    /// component, matching what `serialize_entity` writes only because these
    /// worlds hold nothing outside A–D.
...
/// Deserializing a serialized world reproduces it exactly. `ds` is the
/// tracker the restored `D`s are counted by: deserializing a `D` goes
/// through `DSeed`.
...
fn check_truncation<F: Format>(bytes: &[u8], cut: usize, ds: &DropTracker) {
    ...
    // The fully parsed world was dropped with its components, so this is the
    // floor a leak-free failed parse must come back down to.
...
/// A truncation point strictly inside a stream of `len` bytes. Even an empty
/// world serializes to at least a length marker, so the subtraction cannot
/// underflow.
fn cuts(len: usize) -> impl gs::PrintableGenerator<usize> {
    gs::integers::<usize>().max_value(len - 1)
}
...
fn row_truncated_input_is_rejected_without_leaking(tc: hegel::TestCase) {
    ...
    let cut = tc.draw(cuts(bytes.len()));
    check_truncation::<RowFormat>(&bytes, cut, &ds);
}

/// A failed column parse leaves the components it had already read in a
/// `ColumnBatchBuilder` that is then dropped, which used to leak them all.
// https://github.com/Ralith/hecs/issues/450
#[hegel::test(settings())]
fn column_truncated_input_is_rejected_without_leaking(tc: hegel::TestCase) {
    let ds = DropTracker::new();
    let mut history = tc.draw(histories(0, WORLD_SIZE));
    // Dropping an unbuilt builder whose type includes the zero-sized `C` is a
    // zero-size alloc in `ColumnBatchBuilder::drop`, which Miri rejects
    // (hecs#467, pinned in column_batch.rs), so the Miri worlds stay
    // `C`-free.
    if cfg!(miri) {
        for step in &mut history {
            if let Step::Spawn(cs) = step {
                cs.c = false;
            }
        }
    }
...
/// flipping bits of a payload byte yields a different valid stream. A panic
/// inside the deserializer fails the test as itself. Both outcomes must
/// account for the components parsed along the way, which covers error paths
/// that truncation never reaches.
fn check_corruption<F: Format>(bytes: &[u8], ds: &DropTracker) {
    let baseline = ds.live();
    match F::world(bytes, ds) {
        Ok(restored) => {
            ...
            assert_eq!(
                ds.live(),
                baseline + d_in(&restored),
                "a parsed corrupted stream leaked or double-dropped a component"
            );
        }
        Err(_) => assert_eq!(
            ds.live(),
            baseline,
            "a rejected corrupted stream leaked a component"
        ),
    }
}

/// Not run under Miri: a corrupted entity id makes `spawn_at` grow the
/// metadata table to that id (a flip in the id's high byte demands a few
/// hundred megabytes), which is far beyond what the interpreter can track in
/// reasonable time.
...
/// used to reach an out-of-bounds write and then stale location metadata in
/// `spawn_column_batch_at`. Not run under Miri, for the same reason as its
/// row counterpart: `entity_count` drives a `reserve` before it is validated.
...
/// semantics `spawn_column_batch_at_redundant` in src/world.rs pins. This
/// input used to reach `Archetype::remove(u32::MAX)` and later to leave the
/// entity moved by the deduplication with a stale location index, so `get` on
/// its handle read another entity's component. The duplicated id is nonzero
/// because id 0 named the one row where the stale index was coincidentally
/// correct.
// https://github.com/Ralith/hecs/issues/449
// https://github.com/Ralith/hecs/issues/465
#[test]
fn column_deserialize_tolerates_repeated_entity_ids() {
    let ds = DropTracker::new();
    let bits0 = 1u64 << 32; // generation 1, id 0
    let bits1 = (1u64 << 32) | 1; // generation 1, id 1
    let bytes = [
        encode(&1u64), // one archetype
        encode(&3u32), // entity_count = 3
        encode(&1u32), // component_count = 1
        encode(&0u32), // Id::A
        encode(&bits1),
        encode(&bits0),
        encode(&bits1), // id 1 twice
        encode(&7i32),
        encode(&8i32),
        encode(&9i32),
    ]
    ...
    let e0 = Entity::from_bits(bits0).unwrap();
    let e1 = Entity::from_bits(bits1).unwrap();
    assert_eq!(
        world.len(),
        2,
        "the repeated id must collapse to one entity"
    );
    let fp = fingerprint(&world);
    assert_eq!(fp.get(&e0).and_then(|cs| cs.a), Some(8), "the unique id");
    assert_eq!(
        fp.get(&e1).and_then(|cs| cs.a),
        Some(9),
        "the last row must win"
    );
    assert_eq!(
        world.get::<&A>(e1).ok().map(|a| a.0),
        Some(9),
        "get on the surviving handle"
    );
