...
use fixtures::{handle_from, settings, val, MAX_ENTITIES};
use hecs::{ChangeTracker, Changes, Entity, EntityBuilder, World};
use hegel::generators as gs;
use hegel::stateful::{pool, Pool};
use hegel::TestCase;

/// Rule applications per generated run. Runs stay long so many polls happen
/// from one state. The spawn rules keep the world within `MAX_ENTITIES` so
/// handles still collide.
const STEPS: i64 = 50;
...
struct TrackerModel {
    ...
    /// Every handle ever handed out, despawned ones included, so operations
    /// against dead entities stay common.
    handles: Pool<Entity>,
    /// The `Arc` every `Tracked` clones.
    live: Arc<()>,
}
...
/// Pull one element of `added()` and let the iterator's drop drain the rest.
...
impl TrackerModel {
    ...
    fn spawn(&mut self, tc: TestCase) {
        // Rejects the step while the world is full. Despawn and clear make
        // room again.
        tc.assume(self.model.len() < MAX_ENTITIES as usize);
    ...
    /// A homogeneous batch, so `spawn_batch` also feeds the tracker. Each row
    /// gets a distinct payload, so components swapped between batch entities
    ...
    fn spawn_batch(&mut self, tc: TestCase) {
        ...
        let live = &self.live;
        let row = |i: u32| {
            let value = v.wrapping_add(i as i32);
            (
                Tracked {
                    value,
                    _live: live.clone(),
        ...
        let handles: Vec<Entity> = self.world.spawn_batch((0..n).map(row)).collect();
    ...
    fn write_the_same_value(&mut self, tc: TestCase) {
        let e = self.draw_handle(&tc);
        let current = self.model.get(&e).and_then(|m| m.current);
        // Reject the step, rather than spend it as a no-op, when the entity
        // has no Tracked value to rewrite.
        tc.assume(current.is_some());
        self.world
            .get::<&mut Tracked>(e)
    ...
    fn poll(&mut self, tc: TestCase) {
        ...
        // A possibly-empty subset of the three reports, in a random order.
        // When it is empty, `Changes::drop` does all the draining.
        let pulled = tc.draw(gs::samples(&REPORTS[..]).without_replacement());
        // Only drawn when added() will be pulled, so unused draws do not
        // clutter failure reports.
        let peek_added_only = pulled.contains(&Report::Added) && tc.draw(gs::booleans());
