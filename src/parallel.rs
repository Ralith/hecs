...
/*
This is an alternative parallel iteration approach.  Personally even though it does not match
the style of the iterator based solutions, it is hugely better for many reasons.  Primary among
the benefits is far less unsafe behavior, it is tied to the world dynamically meaning that it
does not have to carry references into the parallel execution graphs and it's a ton less code.
 */
...
/// An opaque parallel iteration handle.
#[derive(Debug, Clone)]
pub struct ParIter(Arc<AtomicU64>);
